...
void main() {
	if (u_blendCount > c_0) {
		...
		gl_Position = u_modelViewProjectionMatrix * (pos+vec4(normal*u_haloScale, 0.0));
	} else {
		gl_Position = u_modelViewProjectionMatrix * (a_position+vec4(a_normal*u_haloScale, 0.0));
	}
	v_uv = a_uv;
}
